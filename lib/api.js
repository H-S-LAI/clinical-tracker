...
// Pearl operations
export async function updatePearl(pearl_id, data) {
  return update('pearls', { pearl_id: `eq.${pearl_id}` }, data);
}

export async function deletePearl(pearl_id) {
  return remove('pearls', { pearl_id: `eq.${pearl_id}` });
}

export async function uploadPearlImage(file) {
  const ext = file.name.split('.').pop();
  const filename = `${Date.now()}.${ext}`;
  const res = await fetch(`${SUPABASE_URL}/storage/v1/object/pearl-images/${filename}`, {
    method: 'POST',
    headers: {
      'apikey': SUPABASE_KEY,
      'Authorization': `Bearer ${SUPABASE_KEY}`,
  ...
  return `${SUPABASE_URL}/storage/v1/object/public/pearl-images/${filename}`;
}
